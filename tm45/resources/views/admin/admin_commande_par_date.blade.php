...
{{-- les contenues --}}
@section('content')
    <h1>Liste de toutes les commandes pour la date : {{$date_verif}}</h1>
    <br>
    <p>Description : Vous êtes sur l'affichage avancée de toutes les commandes pour la date : {{$date_verif}}</p>
    ...
    <table>
        ...
        @foreach ($commandes as $c)
            @if ($c->created_at->format('Y-m-d') == $date_verif)
                <tr>
                    <td>{{$c->id}}</td>
